...
**价值**：关联查询从跨表 JOIN 变为标准语义查询，且命中 BA-BR-06（2 跳内存在已确认欺诈主体则风险分 +30）时可直接返回证据路径。
...
| DA-T-03 | risk_case | case_id PK, subject_ref, status(状态机), risk_score, current_agent, context_json, version(乐观锁) | AA-AG-01 写；全体 Agent 读 | 状态流转经 RocketMQ 事件；并发写入用乐观锁（version）防覆盖 |
...
| DA-T-09 | kb_document | doc_id PK, category(case/regulation/runbook), title, content, status(pending/published) | AA-AG-05 申请写，人工发布 | 发布须人工（BA-BR-11） |
...
| DA-T-12 | agent_memory | memory_id PK, agent_id, case_id, summary, ts | 各 Agent 写 | Agent 长期记忆（赛题 2.4 能力项）；**写时机**：各 Agent 阶段结束时写执行摘要；**读时机**：AA-SK-02 根因匹配与 AA-SK-05 复盘时检索同主体同类记忆作辅助上下文 |
...
设计约束（赛题 2.4：RAG 作为上下文能力而非独立组件）：

1. MCP 接入数据源（AA-MCP-01 `submit_kb_application`）、Skill 封装检索与证据对齐（AA-SK-02/05）、Agent 判断检索结果是否足以支撑决策——严格遵循赛题对 RAG 的定位；
2. 检索结果必须携带 doc_id 引用，写入 DA-T-05 证据表，保证结论可回溯；
3. 未命中知识库时 Agent 必须显式声明"无先例依据"，禁止凭空生成手法结论（防幻觉硬约束）。
...
3. **保留策略**：事件与交易数据 5 年，超期脱敏归档（BA-BR-12）；审计日志与 Trace 保留 3 年；
...
- 配置快照 DA-T-11 由 Nacos（TA-C-05）下发镜像，用于审计回放。

---

## 9. DDD 战术设计（聚合、领域事件与不变量）

战略层面的限界上下文见 [01 §10](./01-业务架构BA.md#10-ddd-战略设计限界上下文与上下文映射)；本节定义各上下文内部的战术构件，是复赛代码结构与 BDD 场景（[06](./06-BDD-TDD工程验证体系.md)）的设计依据。
...
统一经 RocketMQ Topic `case-events` 流转，Tag 即事件类型（与 TA-C-06 对齐）；消息体含 `case_id / trace_id / occurred_at / payload`。人类操作类事件（Approval*）由 web-api（04 §10.1）代人类发布，actor 字段记录真实操作人。

| 事件 | 发布方 | 订阅方 | 触发动作 |
|---|---|---|---|
| CaseRegistered | 主控 Agent | 信号聚合 Agent | 启动 BA-BP-02 聚合（AA-CL-01/02） |
| SignalsAggregated | 信号聚合 Agent | 主控 Agent | 风险分级裁决 |
| InvestigationRequested | 主控 Agent | 调查取证 Agent | 启动 BA-BP-03 |
| InvestigationCompleted | 调查取证 Agent | 处置执行 Agent、审批门户 | 生成处置建议/发起审批 |
| ApprovalApproved / ApprovalRejected | 人类审批官 | 处置执行 Agent | 执行或回滚（BA-BR-07） |
| DispositionExecuted | 处置执行 Agent | 合规审计 Agent | 启动核验（BA-BR-08 计时） |
| VerificationPassed / VerificationFailed | 合规审计 Agent | 主控 Agent、值班房间 | 结案或 P0 升级 |
| CaseArchived | 主控 Agent | 审计 Agent（知识沉淀任务） | 启动 BA-BP-04 |

> **事件分类声明（Sprint 0 落地补充）**：上表 10 种为经 RocketMQ 流转的跨 Agent 领域事件。应用层状态机（web-api core/state_machine.py）另含 7 个**状态机内部迁移事件**，不投递 Topic、仅驱动单事件内迁移与审计留痕：AggregationStarted（聚合承接）、NoiseDismissed（降噪放行）、DispositionSubmitted（处置提交）、RollbackToReview（驳回回滚）、RollbackExecuted（反向处置完成）、ReviewConfirmed/ReviewDismissed（人工复核，API-W-07 人类入口）。两类事件共用 08 §2 CaseStatus 枚举。
