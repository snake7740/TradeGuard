...
| 集成测试 | RocketMQ 状态流转端到端、PolarDB 权限矩阵（03 §6，DA-INV-05）、RAG 检索命中与引用对齐 | Testcontainers 或 docker compose 测试环境 | 覆盖全部 10 种领域事件的发布-订阅 |
| 场景测试（E2E） | SC-01～SC-11 全量，经真实 AgentTeams + MCP 链路执行 | BDD runner 驱动 + Matrix 房间断言 | 11/11 通过为复赛验收线 |
| 评估测试 | BA-KPI-02/03/04 离线评估（召回率/误报率/人工介入率） | 基于 Trace 的评估脚本（04 §7） | 固化于仓库，可复现 |
...
**完备性声明**：14 条 BA 规则中，阈值类规则（BR-05/06）由评分单元测试覆盖，BR-12 数据保留为运维策略不在测试范围；其余规则均有场景或单元级测试承载；6 条 DA 不变量 100% 有测试映射。
