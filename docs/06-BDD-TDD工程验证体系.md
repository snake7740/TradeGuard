...
| 场景测试（E2E） | SC-01～SC-11 全量，经 compose 活栈真实链路执行（Sprint 8 实现修订：pytest 场景矩阵 `tests/test_scenario_matrix.py` 承载断言，demo_playbook 以真实 HTTP + MCP 回放同源剧本，未采用 Matrix 房间断言） | pytest + 活栈 HTTP/MCP 探针 | 11/11 通过为验收线 |
...
| 验收 | 11/11 场景通过 + 评估脚本输出 KPI 报告 + Demo 剧本与场景测试同源（演示即测试回放） |
