...
| BDD | 把业务规则写成 Given-When-Then 可执行验收场景，作为初赛方案的行为规格、复赛代码的验收标准 | SC-01～SC-11 场景目录 | 本文档 §2 |
| TDD | 红-绿-重构循环实现 Skill/状态机/MCP 契约，测试金字塔守护闭环 | 测试金字塔 + 追溯矩阵 | 本文档 §3/§4 |

对赛题的价值：评审要求"端到端闭环证据"——BDD 场景即闭环的行为化证据，TDD 保证该证据可重复执行而非演示一次性脚本。
...
Feature: BA-CAP-07 知识沉淀
  Scenario: SC-05 Agent 不得直接发布知识
    ...
    Then 状态变为 published，向量化入库，AA-SK-02 检索可命中
```
...
| 集成测试 | 事件发布-订阅端到端（进程内总线必达 + RocketMQ 尽力而为，03 §9.2）、PolarDB 权限矩阵（03 §6，DA-INV-05）、RAG 检索命中与引用对齐 | docker compose 活栈测试环境（先起栈后 pytest） | 覆盖全部 21 事件名的发布侧语义 |
| 场景测试（E2E） | SC-01～SC-11 全量，经 compose 活栈真实链路执行（Sprint 8 实现修订：pytest 场景矩阵 `tests/test_scenario_matrix.py` 承载断言，demo_playbook 以真实 HTTP + MCP 回放同源剧本，未采用 Matrix 房间断言） | pytest + 活栈 HTTP/MCP 探针 | 11/11 通过为复赛验收线 |
| 评估测试 | BA-KPI-01~05 离线评估（响应时效/召回率/误报率/人工介入率/留痕完整率） | `scripts/kpi_report.py` 双口径（全量/演示）分列判定，落盘 docs/reports/ | 固化于仓库，可复现 |

**先测后码原则**：Skill 与状态机代码必须先有失败测试（红）再实现（绿）；LLM 相关断言采用"行为断言"（输出结构、引用 doc_id 存在性、审批门控触发）而非文本精确匹配，规避 LLM 不确定性。
...
**完备性声明**：14 条 BA 规则中，阈值类规则 BR-06 由评分单元测试覆盖，BR-05 高频异常规则由专项测试（tests/test_br05_high_freq.py）覆盖命中/未命中/阈值可配置三个分支，BR-12 数据保留为运维策略不在测试范围；其余规则均有场景或单元级测试承载；6 条 DA 不变量 100% 有测试映射。
...
| 阶段 | DoD（完成定义） |
|---|---|
| 初赛 | BDD 场景目录（本文档 §2）作为方案行为规格提交；不要求代码 |
| 复赛 M1 | 最小闭环：Manager 分派→Worker 调 MCP→状态流转；契约测试 + SC-01 通过 |
| 复赛 M2 | 审批链路：SC-02/SC-03 通过；状态机与幂等单元测试全绿 |
| 复赛 M3 | 知识与审计：SC-05/SC-08 通过；集成测试全绿 |
| 决赛 | 11/11 场景通过 + 评估脚本输出 KPI 报告 + Demo 剧本与场景测试同源（演示即测试回放） |

**纪律**：任何场景测试失败不得发布；演示 Demo 的剧本数据与场景测试夹具同源，保证"现场演示 = 已验证行为"，杜绝演示专用旁路代码。
