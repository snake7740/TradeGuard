...
| 失败处理 | 先 APPROVED/INVESTIGATING→DISPOSING 再执行；失败按错误分类重试（可重试错误退避 2 次；幂等冲突 E-IDEMPOTENT-CONFLICT 按成功处理，route=executed 返回首次凭证；门控类错误 E-DISP-AUTH/E-DISP-SCOPE/E-EVIDENCE-MISSING 不重试）；重试耗尽→DISPOSITION_FAILED→MANUAL_REVIEW；幂等键防重复扣冻 |
| 安全边界 | 无 approval_ref 且触及 BA-BR-01/02 边界→直接拒绝执行并返回 E-DISP-AUTH；40≤风险分<70 中风险段无凭证非 release→E-DISP-SCOPE（与 mcp-core 门控同源） |
...
- **逆动作对授权（DA-INV-03 延伸，Sprint 8 加固）**：反向处置（如解冻）以原审批凭证为授权依据——**人批准了冻结，即包含对"冻结出错的纠正（解冻）"的授权，回滚是审批生命周期的组成部分**（02 §3.3 人机边界的自洽延伸：纠错权不超出原授权范围）。mcp-core 验真条件：`decision='approved' AND case_id 匹配 AND (requested_action 匹配 OR 逆动作对匹配)`，逆对 = freeze→release、block→release、reduce→release、release→block（与 mcp-core/verification INVERSE_ACTION 同源）；缺此豁免则核验不一致分支名存实亡（SC-03/07 与剧本 D3 全塌）；
