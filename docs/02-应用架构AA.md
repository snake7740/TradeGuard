...
| 安全边界 | 无 approval_ref 且触及 BA-BR-01/02 边界→直接拒绝执行并返回 E-DISP-AUTH；40≤风险分<70 中风险段无凭证非 release→E-DISP-SCOPE（与 mcp-core 准入逻辑同源） |
...
| AA-CL-02 | 任务拆解 | 按状态机（21 迁移）自动拆解为聚合→调查→处置→审计子任务；聚合裁决四路由（noise/auto_release/investigate/all_fail）决定下一跳 |
