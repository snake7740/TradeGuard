...
**赛题映射声明**：角色编排=Manager/Worker 创建配置；任务拆解=Manager 分派逻辑；上下文传递=房间消息+DA-T-03；协同执行=Worker 并行处理多事件；状态追踪=RocketMQ 状态事件+房间历史。

**自定义 Skill 装载路径与降级声明**：自定义 Skill 经 Nacos Skills Registry 注册后在 Worker 创建时声明装载；若 AgentTeams 当前版本对自定义 Skill 装载能力有限，降级路径为将 Skill 逻辑内置于 Worker 可用工具集（MCP 工具 + Prompt 级能力声明），Skill 清单与 9 属性定义不变（与 [05 §3.4](./05-追溯矩阵与整体评审报告.md) 残余风险第 2 项一致，复赛实测后定稿）。
...
> **落地实况（2026-08-14 实测，如实声明）**：上表为生产形态治理设计；**当前演示环境为旁路直连**，原因与取证如下：
> 1. Higress all-in-one 2.2.3 容器已部署且控制台可达（宿主 :8001 返回 200），但控制台 REST API 一律 401（Login required），且该版本登录端点（/v1/user/login、/v1/session 等）均 404，无法以脚本完成初始化与路由注册；
> 2. 该镜像数据面仅监听 8443（HTTPS 自签证书，容器内实测 200），不监听 80——compose 的 `8180:80` 映射落空（宿主 :8180 无响应）；
> 3. 曾尝试经 standalone 文件存储下发路由（McpBridge dns registry + Ingress 写入 /data 并重启容器验证），路由未被数据面拾取（Host 探测仍 404），已恢复原始配置，不遗留半成品。
>
> 因此演示流量路径为：浏览器→`localhost:8200`→web-api 容器；Agent/脚本→`8101/8102` MCP 直连。所有调用均为真实 HTTP/SQL 链路，仅入口未经网关治理。切换生产形态只需：网关完成路由注册（web-api/MCP 三服务）+ 客户端 base URL 指向网关入口，应用代码零改动（入口无关性由 02 §3 端口适配层保障）。
...
| 维度 | 设计 |
|---|---|
| 数据采集 | AgentScope SDK 自动埋点：LLM 调用、工具调用、Agent 消息；语义遵循 OpenTelemetry（赛题 2.3 推荐标准），业务属性（case_id、agent_id、skill_id）写入 span attribute |
| 数据类型 | Trace（主）+ Log（审计联动）+ Metrics（时延/成功率/Token 消耗）——覆盖赛题"至少 1-2 类" |
| 存储检索 | Studio 内置（演示规模）；生产可导出 OTLP 至任意后端 |
