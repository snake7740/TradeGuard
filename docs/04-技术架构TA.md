...
| **状态追踪** | RocketMQ 状态事件 + 房间历史 + AgentTeams 控制台 | 案件状态机（12 态/18 事件/21 迁移）经 RocketMQ case-events 流转（TA-C-06）；房间历史 + DA-T-08 append-only 审计双留痕；技能 span 经 OTLP 上报 Studio 按案件分组；Manager 控制台/dashboard 运维观测 | demo_playbook 3/3（24 步断言）；Studio span_table 按案件分组落库；dashboard 容器常驻 |
