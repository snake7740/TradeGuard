...
`web-portal` 容器内双进程：**Vue 3 前端静态资源** + **web-api 后端**（Python FastAPI，与 Agent 侧同一代码仓、同一套领域模型）。三端调用链均为真实链路：

```
浏览器(Vue 3) --HTTPS--> web-api(FastAPI) --SQL--> PolarDB-PG（真实读写）
                              |
                              +-- 只读复杂查询：经 mcp-core 工具层（复用 query_audit_trail 等，与 Agent 同源同 Schema）
                              +-- 写操作：事务写 PolarDB 后向 RocketMQ 发布领域事件（如 ApprovalApproved）
                              +-- 实时推送：订阅 case-events Topic → SSE 推送前端（事件流实时刷新）
```
...
| `/api/events/stream` | GET(SSE) | 订阅 case-events 实时推送，演示时事件流转现场可见 | 全体页面 |
