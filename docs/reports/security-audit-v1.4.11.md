...
| 5 | 注入与输入验证 | mcp-core 处置准入、web-api 输入契约、scripts SQL 面 |
...
快照 tar.gz，内置密钥扫描闸门：检出 LLM key/PEM 私钥块/Key 赋值模式即删除导出件并报错）；
...
两次独立连接，Windows 上累计可达秒级）被 2s 轮询捕获后自动链路驱至 DISPOSED，
...
| demo_playbook D1~D3 | **3/3 场景 24 步校验全绿**（令牌强制下业务链完整） |
...
| 级别 | 问题 | 修复 |
| --- | --- | --- |
| **CRITICAL** | starlette <1.0.1 的 CVE-2026-48710：`request.url.path` 可被恶意 `Host: x@evil.com/` 头污染为 `//api/...`，绕过 `api_guards` 的 `startswith("/api/")` 判断而路由仍匹配 → 全量未鉴权访问（第一轮 bearer 守卫被底层框架缺陷旁路） | 双层设防：①守卫改取 `request.scope["path"]`（与路由同源，不受 Host 头影响）；②依赖升级 fastapi==0.141.1 / starlette==1.6.0（CVE 全修复线，一并覆盖 CVE-2025-62727 Range DoS / CVE-2026-54283 表单大小 / CVE-2025-54121 multipart DoS）；mcp-core / mcp-external-mock requirements 显式钉 starlette==1.6.0 |
| **HIGH** | 卷导出件 higress-data.tar.gz 会打入容器内自签证书私钥（data/secrets/ 与 *.key/*.pem），违背"私钥不落盘/不入库"红线 | `volume_export.py` 增 tarfile filter 剔除 `secrets` 目录与 .key/.pem/.crt/.p12/.pfx/.bak；密钥扫描闸门补 base64 封装 PEM（`LS0tLS1CRUdJTi`）/GitHub 令牌（ghp_/github_pat_）/AWS AKIA 特征；再生成导出件实证 higress tar 压缩后仅 6 KB、密钥扫描 0 检出 |
| **HIGH** | AgentTeams 4 个 Worker 的 8088 控制台以非回环（宿主通配/随机）形式发布，暴露管理面 | `agentteams_doctor.py` 固化控制台回环范围：aa-ag-0N → `127.0.0.1:(18090+N)`=18092~18095；新增 `ensure_console_loopback()` 自检——发现非回环发布即重建容器收口（env 经临时文件传入、用后即删，不落 argv/不回显），实测 4/4 回环 OK、RESULT: OK |
...
| 导出件密钥扫描 | 再生成后 higress-data.tar.gz 与 tradeguard-data.sql.gz 均 0 检出 |
| 迁移幂等 | 对运行库重跑 01/04 零改动（见 4.1） |
| Worker 控制台 | doctor 复跑 4/4 回环 OK |
| pytest 全量回归（compose 全栈并行、EventWorker 活跃） | **169/169 全绿**（8m00s）；其间加固 1 处 flake（test_repositories 尾随"状态相等"校验被 EventWorker 抢跑 → 改"审计链无 ApprovalApproved 迁移痕"，竞态免疫） |
| demo_playbook D1~D3 | **3/3 场景 24 步校验全绿** |
...
安全面清零；业务功能与技术调用链零破坏（169/169 全量回归 + 3/3 演示场景于 compose 全栈并行下复跑通过）。**
...
3/3 演示场景、21/21 克隆式冷启动取证，均于 compose 全栈并行下复跑通过）。
