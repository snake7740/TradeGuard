...
| 里程碑 | Sprint | 内容 | 出口标准（对齐 06 §5） |
|---|---|---|---|
| M1 最小闭环 | S1（E1+E2）、S2（E3） | 底座 + 数据 + 聚合放行 | SC-01 通过；契约测试绿 |
| M2 审批链路 | S3–S4（E5） | 处置 + 审批回滚 + 时效 | SC-02/03/07/09/10 通过；状态机/幂等单测全绿 |
| M3 调查与知识 | S5–S6（E4+E6+E7-01~03） | 调查、审计、知识、门户 | SC-05/08 通过；集成测试全绿 |
| 决赛 | S7（E7-04~05） | 可观测 + 演示剧本 | 11/11 场景通过 + KPI 报告 |
