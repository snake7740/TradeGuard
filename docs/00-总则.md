...
| 文档体系版本 | v1.4.22（实现落地阶段：DDD/BDD/TDD 工程验证体系 + 敏捷/API/数据字典交付体系 + 社区对标借鉴 + 全栈实现回写 + 核心技术栈连通攻坚 + 一键启动真实取证 + 文档×代码×HTML 三端一致对齐 + 表述专业化 + 深度安全加固 + 务实化提升（边界契约/AI 实质/工程韧性/风控模型/团伙发现/CI 门禁/离线评估）+ 浏览器多角色协同实测 + 行业对标与补齐差距迭代计划与术语漂移治理 + G1-1 pyod 接线闭环，见 §6 修订记录） |
...
| 文档 | 回答的问题 | 关键产出 |
| --- | --- | --- |
| [01 BA](./01-业务架构BA.md) | 做什么业务、为谁做、按什么规则做 | 业务能力地图、4 条端到端流程、28 条业务规则、业务 KPI |
...
| [06 BDD/TDD](./06-BDD-TDD工程验证体系.md) | 行为如何被验收、代码如何被验证 | 27 个 Given-When-Then 场景、测试金字塔、场景-规则-测试追溯矩阵、分阶段 DoD |
| [07 敏捷/API](./07-敏捷拆分与API契约.md) | 怎么拆交付、接口长什么样 | 7 Epic/29 Story（主线，增强 US-E8~19 见 14 §5）、Sprint 计划、OpenAPI 3.0 契约（32 REST 路径 + 19 MCP 工具） |
| [08 数据字典](./08-数据模型与数据字典.md) | 字段级数据契约 | 17 表字段字典（DA-T-01~17）、10 类枚举、索引字典、错误码表、事件 Payload |
...
### 5.1 路线图别名映射表（防术语漂移，v1.4.21 新增）

docs/09/13 路线图沿用 A1~F3、G1~G5 历史增强点编号，**不属于 §3 权威编号体系**，仅作检索别名；任何落地执行与验收一律以右侧权威编号锚定，防止同物异名导致理解与执行漂移：

| 路线图别名 | 业务含义（业务视角） | 权威锚点（已落地） | G 批次承接（待落地，落地时须补权威编号） |
| --- | --- | --- | --- |
| A1 | 每个账户用自己的历史行为当标尺，偏离才告警（减少误伤老客户） | BA-BR-15 + DA-T-14 | — |
| A2 | 多一位 AI 参谋给值班员递异常提示（只参谋不定案） | — | G1-2 |
| B1 | 看资金关系形状识别团伙（星型归集/环形洗钱），只给线索不下结论 | BA-BR-16 + DA-INV-07 | G5-1 深化 |
| B2 | 三类洗钱节奏识别（资金回路/快进快出/夜间突发） | BA-BR-17 | — |
| B3 | 多条怀疑线索并行查证，防止只查一个方向漏掉真因 | BA-BR-18 | — |
| C1 | 处置前控辩双方互审，防止过度冻结误伤 | BA-BR-19 + DA-INV-09 | — |
| C2 | 处置后 7/30 天回头看效果，处置好坏可度量 | disposition_outcome 回填 | — |
| D1 | 审计前自动预检材料齐不齐，审计员只复核缺口 | API-W precheck 端点 | — |
| D2 | 一键生成带出处的案件审计摘要 | — | G3-1 |
| E1 | 知识条目新陈代谢，没人用的自动降级 | effectiveness_score | — |
| E2 | 从误判案件提议新规则，人审后才生效 | BA-BR-21 + DA-INV-08 | — |
| F1 | 金额序列统计异常检测（隔离森林/LOF/ECOD，仅参谋分） | 已接线（G1-1，2026-08-21） | BA-BR-25 / SC-24 / API-M-17~19 / US-E16（调查第五可选源 stat，仅无特征保守全查路径，advisory 不裁决） |
| F2/F3 | 技能包社区发布与第三方框架互操作实证 | ecosystem-interop-report.md | G4-3 |

**执行纪律**：G 批次落地时须先在 01/06/07 补齐 BA-BR/SC/API-M 权威编号，再开工；文档与对话引用同一能力时优先用权威编号，别名仅用于路线图检索。
...
| v1.4.21 | 2026-08-21 | 行业对标与补齐差距迭代计划入体系 + 术语漂移治理：docs/09 v1.2（代码实证盘点 + 行业核实数据口径区分 + 五维水准定位 + 诚实披露）、docs/13 v2.1（A1~F3 落地盘点 + G1~G5 补齐差距迭代计划 + v2.1 复核补遗 G1-4/G4-3 与差距覆盖复核矩阵）；新增 §5.1 路线图别名映射表（A1~F3 历史别名→业务含义→权威锚点→G 批次承接），纪律：G 批次落地须先补 BA-BR/SC/API-M 权威编号再开工，防同物异名导致理解与执行漂移 |
| v1.4.22 | 2026-08-21 | G1-1 pyod 接线闭环落地（US-E16）：F1 行更新为已接线（BA-BR-25 / SC-24 / API-M-17~19）——金额序列统计离群检测成为调查第五可选源 stat（仅无特征保守全查路径，advisory 参谋不裁决，依赖缺失/样本不足降级留痕不阻断）；全量回归新基线 30 文件 322 例全绿 |
