...
| execute_disposition | API-M-03 | 处置执行 | 审批门控 approval_ref 验真（case 匹配 + requested_action 匹配或逆动作对，DA-INV-02/03）；幂等键 E-IDEMPOTENT-CONFLICT；中风险段 E-DISP-SCOPE；release 豁免仅 ROLLBACK 态；70 分线经 sys_config 同源 |
| query_disposition_result | API-M-04 | 处置结果回查 | 只读（AA-SK-04 核验依据），未命中返回 E-NOT-FOUND |
...
| query_sentiment | API-M-08 | 舆情模拟（命中概率 0.3，confidence 0.4-0.9） |
...
- **确定性模拟**：`_seed(prefix+subject)` = `random.Random(int(sha256(prefix+subject)[:8],16))`，
  同一主体多次查询结果一致（演示可复现，剧本主体经本地探针预选即复刻此播种）；
