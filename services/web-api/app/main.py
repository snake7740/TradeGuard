...
from .api import (alerts, approvals, audit, cases, config, demo, events_stream,
                  health, kb, observability)
...
from .skills.disposition import (ESCALATION_MINUTES, DispositionService,
...
from .skills.verification import (VERIFICATION_MINUTES, VerificationService,
...
async def lifespan(app: FastAPI):
    ...
    app.state.publisher = RocketMQPublisher(
        os.getenv("ROCKETMQ_NAMESRV", "rocketmq-namesrv:9876"), fallback=InMemoryPublisher())
    ...
    app.state.aggregation = AggregationService(
        ...
        config=app.state.config)   # BA-BR-05 阈值热加载
    ...
    if worker_enabled():
        app.state.event_worker = EventWorker(
            pool=app.state.pool, aggregation=app.state.aggregation,
    ...
    app.state.disposition = DispositionService(
        ...
        config=app.state.config)   # SC-06 阈值热加载（D1）
        ...
        while True:
            try:
                await scan_pending_escalations(
                    app.state.pool, app.state.publisher,
                    minutes=_cfg_int("br-13-approval-timeout-min", ESCALATION_MINUTES))
                await scan_verification_overdue(
                    app.state.pool, app.state.publisher,
                    minutes=_cfg_int("br-08-verification-timeout-min", VERIFICATION_MINUTES))
