...
from .core.state_machine import CaseEvent, CaseState, next_state
...
class CaseRepository:
    ...
    async def register(
        ...
        async with self._pool.acquire() as conn, conn.transaction():
            ...
            await conn.execute(
                ...
                actor,
                case_id,
                f"source={source_type},severity={risk_score}",
                trace_id,
            )
        await self._pub.publish(
            case_id,
            "CaseRegistered",
            {"subject_ref": subject_ref, "risk_score": risk_score},
            actor,
            trace_id=trace_id,
        )
    ...
    async def transition(
        ...
        """
        r = await self._pool.fetchrow(
            "SELECT status, version, trace_id FROM risk_case WHERE case_id=$1", case_id
        )
        if not r:
            raise LookupError(case_id)
        target = next_state(
            CaseState(r["status"]), event, actor
        )  # 非法迁移直接抛 InvalidTransition
        async with self._pool.acquire() as conn, conn.transaction():
            ...
            await conn.execute(
                ...
                uuid.uuid4().hex,
                actor,
                f"case.transition.{event.value}",
                case_id,
                # R-37 复审收口：basis 截断对齐 varchar(300)——opinion 等长文本经
                # 迁移 basis 入审计，此前超长值令事务回滚 500（业务操作被审计列宽击穿）
                (basis or f"{r['status']}->{target.value}")[:300],
                r["trace_id"],
            )
        await self._pub.publish(
            case_id,
            event.value,
            {"from": r["status"], "to": target.value},
            actor,
            trace_id=r["trace_id"],
        )
        return {
    ...
    async def list(self, decision: str) -> list[dict]:
        ...
        rows = await self._pool.fetch(
            """SELECT a.*, c.risk_score, c.subject_ref
               ...
               WHERE a.decision=$1 ORDER BY a.created_at""",
            decision,
        )
