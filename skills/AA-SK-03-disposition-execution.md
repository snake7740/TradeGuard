...
2. **建单**：编排层收 E-DISP-AUTH → API-M-11 `create_approval_request` 建审批工单（DA-T-07，requested_action/requested_amount 随单写入，案件不存在→E-NOT-FOUND）+ 案转 PENDING_APPROVAL；
...
SC-02（审批门控）、SC-03（驳回回滚）、SC-07（幂等）、SC-09（时效升级）、SC-10（中风险拒自动处置）。测试载体：services/web-api/tests/test_disposition.py（纳入 Sprint 8 全量回归 169 例全绿）。
